...
describe('Pagination Component', () => {
  test('it disables all buttons when there is one page of features', async() => {
    const PaginationWrapper = () => {
      ...
      return <><Pagination filters={filters} lastPage={1} featureCount={1} setFilters={setFilters} /></>;
    };
  ...
  test('it goes to last page when last page clicked', async () => {
    const PaginationWrapper = () => {
      ...
      return <><Pagination filters={filters} lastPage={3} featureCount={50} setFilters={setFilters} /></>;
    };
    render(<MantineProvider>
        <PaginationWrapper />
        </MantineProvider>);
  ...
  test('it goes to last page when last page clicked and theres an even amount of features', async () => {
    const PaginationWrapper = () => {
      const [filters, setFilters] = useState<Filters>({ s: '', page: 1, count: 40, category: '0' });
      return <><Pagination filters={filters} lastPage={3} featureCount={40} setFilters={setFilters} /></>;
    };
    render(<MantineProvider>
        <PaginationWrapper />
        </MantineProvider>);
